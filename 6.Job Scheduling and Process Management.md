...
```
| Cron Expression | Full Command                        | Meaning                               |
| --------------- | ----------------------------------- | ------------------------------------- |
| `0 0 * * *`     | `0 0 * * * /home/user/backup.sh`    | Runs **daily at midnight (12:00 AM)** |
| `0 6,18 * * *`  | `0 6,18 * * * /home/user/backup.sh` | Runs at **6 AM and 6 PM every day**   |
| `0 */6 * * *`   | `0 */6 * * * /home/user/monitor.sh` | Runs **every 6 hours**                |
| `*/10 * * * *`  | `*/10 * * * * /home/user/script.sh` | Runs **every 10 minutes**             |
| `0 * 20 7 *`    | `0 * 20 7 * /home/user/backup.sh`   | Runs **every hour on July 20**        |
...
• A foreground process runs in the terminal and takes input directly from the user.
...
• Key Characteristics
 ...
 3. Blocks terminal until finished
...
ii. You must wait until it finishes

🔹 B. Background Process / Job

• A background process runs without occupying the terminal, allowing other commands to run.

• Key Characteristics

 1. Runs in background

 2. Does not block terminal

 3. Started using &
 
 4. Managed using jobs, fg, bg

• Example:  sleep 100 &
   

i. Runs in background

ii. Terminal is free immediately

🔹 C. Daemon Process

• A daemon process is a background service that runs continuously without user interaction.
...
• Key Characteristics
 ...
 4. Provides system services

• Example:  sshd

i. Runs in background

ii. Handles SSH connections
...
• Key Characteristics
 ...
 1. Process is finished but not removed

 2. Parent has not read exit status
...
ii. Parent does not collect status
